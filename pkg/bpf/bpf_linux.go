...
func objCheck(m *ebpf.Map, path string, mapType ebpf.MapType, keySize, valueSize, maxEntries, flags uint32) bool {
	...
	if m.Type() != mapType {
		fmt.Println("Warning Map type mismatch for BPF map old/new", m.Type(), mapType)
		mismatch = true
	}

	if m.KeySize() != keySize {
		fmt.Println("Warning Key-size mismatch for BPF map old/new ", m.KeySize(), keySize)
		mismatch = true
	}

	if m.ValueSize() != valueSize {
		fmt.Println("Warning Value-size mismatch for BPF map old/new ", m.ValueSize(), valueSize)
		mismatch = true
	}

	if m.MaxEntries() != maxEntries {
		fmt.Println("Warning Max entries mismatch for BPF map old/new ", m.MaxEntries(), maxEntries)
		mismatch = true
	}
	if m.Flags() != flags {
		fmt.Println("Warning Flags mismatch for BPF map old/new ", m.Flags(), flags)
		mismatch = true
	}
	...
	if mismatch {
		...
		fmt.Println(" Warning Removing map to allow for property upgrade (expect map data loss)")
...
func OpenOrCreateMap(spec *ebpf.MapSpec, pinDir string) (*ebpf.Map, error) {
	...
	if errors.Is(err, ebpf.ErrMapIncompatible) {
		...
		fmt.Println("Unpinning map with incompatible properties")
