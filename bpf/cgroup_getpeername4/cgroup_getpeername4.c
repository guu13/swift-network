...
int cgroup_getpeername4_pod2svc(struct bpf_sock_addr *ctx) {
    ...
    // bpf_printk("cgroup_getpeername4_pod2svc, pod dst_ip:port x.%d.%d.%d:%d", 
    //         (_tmp_dst_ip>> 16) & 0xFF,(_tmp_dst_ip >> 8) & 0xFF, _tmp_dst_ip & 0xFF, __bpf_ntohs(dst_port));
    __u16 tt = 8899;
    __u16 tt2 = 9988 ;
    if(_tmp_dst_port == tt || _tmp_dst_port == tt2){
        ...
        //188207882:1063
        ctx->user_ip4 = 188207882;
        ctx->user_port = 1063;

        _tmp_dst_ip = __bpf_ntohl(ctx->user_ip4);
        _tmp_dst_port = __bpf_ntohs(ctx->user_port);
        bpf_printk("cgroup_getpeername4_pod2svc, svc dst_ip:port %x:%u",ctx->user_ip4, ctx->user_port);
        bpf_printk("cgroup_getpeername4_pod2svc, svc dst_ip:port ntoh %x:%u",_tmp_dst_ip, _tmp_dst_port);
        bpf_printk("cgroup_getpeername4_pod2svc, svc dst_ip:port %d.%d.%d.%d:%d", 
            (_tmp_dst_ip>> 24) & 0xFF, (_tmp_dst_ip>> 16) & 0xFF, (_tmp_dst_ip >> 8) & 0xFF, _tmp_dst_ip & 0xFF, _tmp_dst_port);
        return 1;
    }
